...
updates_buttons () {
    case "$1" in
        "1")
            ...
            # Update the bar
            run_updates > /tmp/lemon_fifo

            # Set a timeout to clear the extra information
            { sleep 3; rm -f $MORE_UPDATES_INFO_FILE; run_updates > /tmp/lemon_fifo; } &
            ;;
        "2")
...
run_updates () {
    ...
    # Print the stuff
    SYMBOL="\uf466"
    [ -f /tmp/statusbar/repoupdate-state ] && read SYMBOL < /tmp/statusbar/repoupdate-state
